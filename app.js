...
const DOMAINS = {
  ...
  blood: {
    ...
    categories: [
      { key: '抗凝固薬',          label: '🩸 抗凝固薬' },
      { key: '抗血小板薬',        label: '🧬 抗血小板薬' },
      { key: '血栓溶解薬',        label: '⚡ 血栓溶解薬' },
      { key: '鉄剤',              label: '🔴 鉄剤' },
      { key: 'ESA（腎性貧血）',   label: '💉 ESA（腎性貧血）' },
      { key: 'G-CSF',             label: '🦴 G-CSF' },
      { key: 'TPO受容体作動薬',   label: '🩸 TPO受容体作動薬' },
    ],
    categoryGroups: [
      { key: 'coagulation',   label: '凝固系', cats: ['抗凝固薬', '抗血小板薬', '血栓溶解薬'] },
      { key: 'hematopoietic', label: '造血系', cats: ['鉄剤', 'ESA（腎性貧血）', 'G-CSF', 'TPO受容体作動薬'] },
    ],
    defaultGroup: 'coagulation',
    defaultCat: '抗凝固薬',
  ...
  antibiotics: {
    ...
    categories: [
      ...
      { key: 'セフェム第5世代・新規',     label: '💊 セフェム第5世代・新規' },
    ...
    categoryGroups: [
      { key: 'byClass',    label: 'クラス別',  cats: ['ペニシリン系', 'セファロスポリン第1世代', 'セファロスポリン第2世代', 'セファロスポリン第3世代', 'セファロスポリン第4世代', 'セフェム第5世代・新規', 'カルバペネム系', 'マクロライド系', 'テトラサイクリン系', 'フルオロキノロン系', 'アミノグリコシド系', 'グリコペプチド系', 'その他'] },
...
function buildDrugCard(d, defs, cfg) {
  ...
  const detailHTML = defs.map(def => {
    ...
    const rowClass = def.type === 'mech'   ? 'card-detail-row card-detail-mech'
                   ...
                   : def.type === 'wound'  ? 'card-detail-row card-detail-wound'
...
const STRONG_OPIOID_ROWS = [
  ...
  { label: '効果発現時間',     field: 'onset_time',      type: 'val'    },
  { label: '効果持続時間',     field: 'duration_hours',  type: 'val'    },
  { label: '等鎮痛換算量',     field: 'equianalgesic',   type: 'mech'   },
  { label: '妊娠｜授乳',       fields: ['pregnancy', 'lactation'], type: 'safety_pair' },
  { label: '腎機能｜透析性',   fields: ['renal_gfr', 'dialysis'], type: 'safety_pair' },
  { label: '使い分けポイント', field: 'guideline_rank',  type: 'usecase'},
  { label: 'エビデンス出典',   field: 'evidence',        type: 'evidence'},
...
// 骨粗鬆症治療薬（全カテゴリ共通）
const OSTEOPOROSIS_ROWS = [
  { label: '主な作用',         field: 'action_type',    type: 'mech'   },
  { label: '作用機序',         field: 'mechanism',      type: 'mech'   },
  { label: '骨折リスク低下',   field: 'placebo_onset',  type: 'accent' },
  { label: 'BMD変化（腰椎）',  field: 'placebo_sleep',  type: 'accent' },
  { label: 'NNT（椎体骨折）',  field: 'NNT',            type: 'nnt'    },
  { label: '効果スコア',       field: 'efficacy_star',  type: 'stars'  },
  { label: '効果発現',         field: 'onset_time',     type: 'val'    },
  { label: '投与法・頻度',     field: 'duration_hours', type: 'val'    },
  { label: '妊娠｜授乳',       fields: ['pregnancy', 'lactation'], type: 'safety_pair' },
...
// 褥瘡治療薬（褥瘡の状態ステージを優先表示）
const WOUND_ROWS = [
  { label: '適応・対象',           field: 'action_type',    type: 'mech'   },
  { label: '適応する褥瘡の状態',   field: 'wound_stage',    type: 'wound'  },
  { label: '作用機序',             field: 'mechanism',      type: 'mech'   },
  { label: '有効性指標',           field: 'placebo_onset',  type: 'accent' },
  { label: '付加効果・特徴',       field: 'placebo_sleep',  type: 'accent' },
  { label: '効果スコア',           field: 'efficacy_star',  type: 'stars'  },
...
// ===== 造血系 ROW_DEFS =====
const BLOOD_HEMA_ROWS = [
  { label: '主な適応',         field: 'action_type',    type: 'mech'    },
  { label: '作用機序',         field: 'mechanism',      type: 'mech'    },
  { label: '有効性指標',       field: 'placebo_onset',  type: 'accent'  },
  { label: '治療目標・安全性', field: 'placebo_sleep',  type: 'accent'  },
  { label: 'NNT',              field: 'NNT',            type: 'nnt'     },
  { label: '効果スコア',       field: 'efficacy_star',  type: 'stars'   },
  { label: '効果発現',         field: 'onset_time',     type: 'val'     },
  { label: '投与法',           field: 'duration_hours', type: 'val'     },
...
function getClassBadge(cls) {
  const map = {
    ...
    'PCSK9阻害薬（siRNA）':           { css: 'pcsk9i-sirna' },
    'フィブラート系':                 { css: 'fibrate' },
    '高純度EPA':                      { css: 'epa' },
    ...
    '抗TSLP抗体（生物学的製剤）':     { css: 'resp-bio-tslp' },
    ...
    '第5世代セフェム':                     { css: 'ab-cef5'   },
    'シデロフォアセフェム':                { css: 'ab-cef5'   },
    'βラクタマーゼ阻害薬配合セフェム':    { css: 'ab-cef3'   },
    'βラクタマーゼ阻害薬配合新規セフェム（MDR緑膿菌特化型）': { css: 'ab-cef5' },
    'βラクタマーゼ阻害薬配合セフェム（CRE対応）':            { css: 'ab-cef5' },
    'カルバペネム':                        { css: 'ab-carba'  },
    'ペネム系':                            { css: 'ab-carba'  },
    'モノバクタム系':                      { css: 'ab-mono'   },
...
const DOSE_CALC = {
  ...
  k: { label: 'カリウム製剤（K量換算）', mme: true, refName: 'カリウム', unit: 'mEq（K⁺）', outUnit: 'mEq',
    note: '本計算機は各製剤の含有カリウム量（mEq）への「同mEq換算」。K⁺はどの塩でも同mEqなら負荷は同じで、製剤の切り替えも同mEqで行い血清K・心電図をモニタリングするのが安全（生理学的にも妥当）。徐放錠1錠600mg＝K 8mEq、L-アスパラギン酸K錠300mg＝K 1.8mEq、グルコン酸Kは製剤ラベルのmEq表記どおり。複数製剤を足すと1日総K量の目安になる。※一部の切替表は「常用量対比（常用量上限どうしを等価とみなす）」で換算し数値が大きく異なる（特に塩化カリウム散で数倍〜）が、根拠が弱く危険とされ同mEq切替が推奨される。塩の種類でCl負荷が異なり、低クロール血症性アルカローシスにはKClが合理的。注射（KCL補正液）は必ず希釈・速度厳守。',
    drugs: {
